      ...
      <div className="mx-auto max-w-5xl px-4 md:px-6">
        ...
        <div className="mx-auto mt-12 max-w-xl">
          <Accordion
            type="single"
            collapsible
            className="bg-card w-full rounded-2xl px-8 py-3"
          >
            {faqItems.map((item) => (
              <AccordionItem
                key={item.id}
                value={item.id}
                className="border mb-4 last:mb-0 rounded-lg px-4"
              >
                <AccordionTrigger className="cursor-pointer text-[#09121D] font-medium text-[16px] leading-[100%] hover:no-underline">
                  {item.question}
                </AccordionTrigger>
                <AccordionContent>
